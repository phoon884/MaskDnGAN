    ...
    with torch.no_grad():
        for index in tqdm(range(len(allImageNames) - numOfFrames + 1)):
            ...
            for imageName in imagesNames:
                ...
                image_raw = pack_gbrg_raw(int(255*np.random.rand(1980*4.1080*4)))#np.array(image_raw)
                print(f"test:{image_raw.shape}")
                image_raw = np.random.rand(1920//2, 1080//2, 4).astype(np.float32)
...
if __name__ == '__main__':
    ...
    backWarp = dataloader.backWarp((1920//2, 1080//2), device)
